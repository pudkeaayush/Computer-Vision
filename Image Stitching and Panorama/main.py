...
def getTransform(src, dst, method = 'affine'):
    ...
    print ("Get transform called")
    if method == 'affine':
        M, mask = cv2.estimateAffine2D(src_pts, dst_pts, cv2.RANSAC, ransacReprojThreshold=5.0)
        #M = np.append(M, [[0,0,1]], axis=0)
        print("Shape is" , mask.shape , M.shape)
...
def Perspective_warping(img1, img2, img3):
       ...
       # then transform img2 with the 3x3 transformation matrix
        ...
        # then transform img3 with the 3x3 transformation matrixi
        out = cv2.warpPerspective(img3, M1, (img1.shape[1], img1.shape[0]), dst = out.copy(), borderMode=cv2.BORDER_TRANSPARENT)
        #warped_image1 = cv2.warpPerspective(img3, M1, dimension_of_warped_image)
...
def Cylindrical_warping(img1, img2, img3):
        print ( "Cylindrical called")
        img1 = cv2.copyMakeBorder(img1,50,50,300,300,cv2.BORDER_CONSTANT)
       
        h,w = img1.shape
        f = 410
        K = np.array([[f, 0, w/2], [0, f, h/2], [0, 0, 1]]) # mock calibration matrix
        imcyl1 = cylindricalWarpImage(img1, K)
         
        h,w = img2.shape
        f = 410
        K = np.array([[f, 0, w/2], [0, f, h/2], [0, 0, 1]]) # mock calibration matrix
        imcyl2 = cylindricalWarpImage(img2, K)

        h,w = img3.shape
        f = 410
        K = np.array([[f, 0, w/2], [0, f, h/2], [0, 0, 1]]) # mock calibration matrix
        imcyl3 = cylindricalWarpImage(img3, K)

        (M , pts1 , pts2 , mask ) = getTransform( imcyl2[0], imcyl1[0], 'affine')
        (M1 , pts11 , pts21 , mask1 ) = getTransform(imcyl3[0], imcyl1[0] , 'affine')    
        # first, make some room around img1
       # then transform img2 with the 3x3 transformation matrix
        img1= cv2.warpAffine(imcyl2[0] , M, (imcyl1[0].shape[1] , imcyl1[0].shape[0]), dst=imcyl1[0].copy() , borderMode = cv2.BORDER_TRANSPARENT) 
        #print (out.shape)
        # then transform img3 with the 3x3 transformation matrixi
        out = cv2.warpAffine(imcyl3[0], M1, ( img1.shape[1] , img1.shape[0]) , dst = img1.copy() , borderMode = cv2.BORDER_TRANSPARENT)
        out_mask = cv2.warpAffine(imcyl3[1] , M1 , (img1.shape[1] , img1.shape[0]), dst = np.zeros_like(img1) , borderMode = cv2.BORDER_TRANSPARENT)
        #warped_image1 = cv2.warpPerspective(img3, M1, dimension_of_warped_image)
        img1 = out * ( out_mask / 255 ) + img1 * ( 1 - ( out_mask / 255))
        print (out.shape)	
	# Write your codes here
	output_image = img1 # This is dummy output, change it to your output	
...
if __name__ == '__main__':
   ...
   # Call the function
   #function_launch[1]()
   example_output1 = cv2.imread('example_output2.png',0)
   output_homography = cv2.imread('output_cylindrical.png',0)
   print(RMSD(output_homography, example_output1))
